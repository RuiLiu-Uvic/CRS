...
// The mobility model is set as 5 with the variable m_mobility. 
// When m_mobility=5, we can set the speed model with "Speed"; The Variance reflects the std variable in the paper.
...
NS_LOG_COMPONENT_DEFINE ("vanet-routing-rui");
...
Masking (int raw_obser, int nodeID)
{
  ...
  NS_LOG_INFO("raw_obser: "<<raw_obser);
  double mask = 0;
...
VanetRoutingExperiment::PrintReceiveList ()
{
  ...
  else
  {
    ...
    {
        obser_received[map_id[i]] = cal_result[i]; //add the calculated result to the observation list
    }
  ...
  cout << endl;
  cout << "Average(ms): "<< 1000.0*average_end_to_end_delay/end_delay.size() << endl << endl;
  ...
  << "," << packet_recovery_rate << "," << 1000.0*average_end_to_end_delay/end_delay.size() << "," <<  (average_masking/masking_time.size())/1000000.0 
...
void VanetRoutingExperiment::SetupLogging ()
{
  ...
  //LogComponentEnable ("vanet-routing-rui",LOG_LEVEL_INFO);
...
VanetRoutingExperiment::SetupAdhocMobilityNodes ()
{
  ...
  else if (m_mobility == 4) //Constant direction moving 
  {     
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
  ...
  if (m_lossModel == 3)
  ...
  else if (m_lossModel == 7) //What we used
    {
      ...
      cout<<"wifiChannel add propagation loss:"<< m_lossModelName <<endl;
      wifiChannel.AddPropagationLoss (m_lossModelName, "MaxRange", DoubleValue (100.0));//100
    }
